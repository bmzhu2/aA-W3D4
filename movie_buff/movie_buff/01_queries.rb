...
def harrison_ford
  ...
  Movie.joins(:actors).select(:id, :title).where("actors.name = ? AND castings.ord != ?", "Harrison Ford", 1)
end
...
def biggest_cast
  ...
  Movie.joins(:castings).select('movies.id, movies.title').group(:id).order('COUNT(castings.actor_id) DESC').limit(3)
